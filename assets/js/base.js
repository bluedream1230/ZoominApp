...
baseurl = "https://app.zoomingaming.com/api/index.php/";
...
function validate(form_id, fun, parameter = false) {
  ...
  $(
    ...
    if ($(this).attr("data-required") == "yes" && $(this).val() == null) {
      $(this)
        ...
        .append(
          '<div class="input-danger">' +
            $(this).attr("data-validation-required-message") +
            "</div>"
        );
      ...
      $(this).attr("data-pattern") !== undefined &&
      !$(this).val().trim().match($(this).attr("data-pattern")) &&
      $(this).val().trim() != ""
    ) {
      $(this)
        .parent()
      ...
      $(this).attr("data-match") !== undefined &&
      $(this).val().trim() !=
        $("#" + $(this).attr("data-match"))
          .val()
          .trim() &&
      $(this).val().trim() != ""
    ) {
      ...
      $(this)
        ...
        .parent()
        .append(
          '<div class="input-danger"> Minimum length ' +
            $(this).attr("minlength") +
            "</div>"
        );
      $(this).parent().parent().addClass("text-danger");
      ...
      $(this).attr("data-unique-fun") !== undefined &&
      $(this).val().trim() != ""
    ) {
        ...
        if (typeof fun_error != "undefined" && fun_error != "ok") {
          if ($(this).attr("data-show-toast") !== undefined) {
            ...
            $(this)
              .parent()
              .parent()
              .append('<div class="input-danger">' + fun_error + "</div>");
            $(this).parent().parent().addClass("text-danger");
          }
          error++;
        } else {
...
function check_email_exist(event, req, form) {
  ...
  $.ajax({
    ...
    data:
      $(form).find("select, textarea, input").serialize() + "&user_id=" + id,
    async: false,
    success: function (html) {
      ...
      if (obj[0]["success"] == true) {
        ...
        $(this)
          .parent()
          .parent()
          .append('<div class="input-danger">' + obj[0]["message"] + "</div>");
        $(this).parent().parent().addClass("text-danger");
      }
...
function login() {
  ...
  $.ajax({
    ...
    data: decodeURIComponent(
      $("#login").find("select, textarea, input").serialize()
    ),
    async: true,
    success: function (html) {
...
function register() {
  ...
  $.ajax({
    ...
    data: decodeURIComponent(
      $("#register").find("select, textarea, input").serialize()
    ),
    async: true,
    success: function (html) {
      ...
      if (obj[0]["success"] == true) {
        if ($("#register input[name=user_id]").val() == "") {
          ...
          setTimeout(function () {
            window.location.replace("login.html?event_id=" + event_id);
          }, 500);
        } else {
          show_toast(
          ...
          setTimeout(function () {
            localStorage.setItem(
              "user_id",
              $("#register input[name=user_id]").val()
            );
            localStorage.setItem(
              "user_email",
    ...
    success: function (html) {
      ...
      for ($i = 0; $i < Object.keys(obj[0]).length; $i++) {
        if (obj[0][Object.keys(obj[0])[$i]] == "0000-00-00") {
          ...
          if (
            $(id + " input[name=e" + Object.keys(obj[0])[$i] + "]").attr(
              "type"
            ) == "radio"
          ) {
            $(
...
function show_toast(message, type, not_type, pos = "top", bg = "inverse") {
  ...
  if (not_type == "big") {
    $("body").append(
      '<div id="toast' +
        random +
        '" class="toast-box text-center toast-' +
        pos +
        ' show"><div class="in"><ion-icon ' +
        $text_type +
        '></ion-icon><div class="text">' +
        message +
        "</div></div></div>"
    );
  } else {
    $("body").append(
      '<div id="toast' +
        ...
        " text-center toast-" +
        pos +
        ' show"><div class="in w-100 p-0"><div class="text">' +
        message +
        "</div></div></div>"
    );
  }
...
function get_event_details(event_id) {
  ...
  $.ajax({
    type: "GET",
    url: api_url + "event_api/" + event_id,
    async: false,
    dataType: "json",
    success: function (html) {
      console.log(html);
      ...
      obj = JSON.parse(obj);
      if (obj.GameName == "Basketball") {
        window.location.replace("dashboard.html?event_id=" + event_id);
      } else {
        window.location.replace("trivia/?trivia_id=" + obj.TriviaId);
      }
    },
  });
