...
class PlayerModel extends CI_model
{
    ...
    public function addEventPlayer()
    {
        ...
        curl_setopt_array($curl, array(
          CURLOPT_URL => 'https://saviour.earth/ZoomIn/api/index.php/Game/get_event/',
          ...
          CURLOPT_POSTFIELDS=> "event_id=".$param['event_id']
        ));

        $response = curl_exec($curl);
        curl_close($curl);
        $response=json_decode($response, true);
        $user_limit=$response[0]['user_limit'];
        $game_start_time = $response[0]['EventStartTimeDate'];
        $duration = $response[0]['Time_Limit'];
      	$start_time = strtotime(date('Y-m-d H:i'));
        $end_time =date('Y-m-d H:i',strtotime($response[0]['EventCompleteTimeDate']));
